...
import { MARKET_2026 as M } from "../../assumptions/market";
...
describe("caution d'un organisme", () => {
  const g = guaranteeCost("suretyship", CAPITAL, PRIX, M);
  ...
  it("coûte une fraction du capital garanti, et non du prix du bien", () => {
    ...
    const surCapitalDouble = guaranteeCost("suretyship", CAPITAL * 2, PRIX, M);
    expect(surCapitalDouble.cost).toBe(g.cost * 2);

    // Changer le prix du bien sans changer le capital ne change rien.
    const autrePrix = guaranteeCost("suretyship", CAPITAL, PRIX * 3, M);
...
describe("hypothèque", () => {
  const g = guaranteeCost("mortgage", CAPITAL, PRIX, M);
  ...
  it("coûte une mainlevée en cas de revente avant terme, assise sur le prix du bien", () => {
    ...
    const bienPlusCher = guaranteeCost("mortgage", CAPITAL, PRIX * 2, M);
  ...
  it("son coût initial porte sur le capital garanti", () => {
    const capitalDouble = guaranteeCost("mortgage", CAPITAL * 2, PRIX, M);
    expect(capitalDouble.cost).toBe(g.cost * 2);
  });
});

describe("nantissement", () => {
  const g = guaranteeCost("pledge", CAPITAL, PRIX, M);
  ...
  it("est la moins chère des trois à l'entrée", () => {
    ...
    const caution = guaranteeCost("suretyship", CAPITAL, PRIX, M);
    const hypotheque = guaranteeCost("mortgage", CAPITAL, PRIX, M);
...
describe("les trois garanties", () => {
  it("portent toutes le type demandé, et rien d'autre", () => {
    for (const kind of ["suretyship", "mortgage", "pledge"] as const) {
      expect(guaranteeCost(kind, CAPITAL, PRIX, M).kind).toBe(kind);
    }
  });

  it("ne produisent ni coût négatif ni restitution supérieure au coût", () => {
    for (const kind of ["suretyship", "mortgage", "pledge"] as const) {
      const g = guaranteeCost(kind, CAPITAL, PRIX, M);
  ...
  it("rendent tout à zéro sur un capital nul", () => {
    for (const kind of ["suretyship", "mortgage", "pledge"] as const) {
      const g = guaranteeCost(kind, 0, PRIX, M);
  ...
  it("restent des entiers de centimes", () => {
    ...
    for (const kind of ["suretyship", "mortgage", "pledge"] as const) {
      const g = guaranteeCost(kind, euros(137_777), euros(212_345), M);
