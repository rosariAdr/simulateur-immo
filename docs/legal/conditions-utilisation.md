# Conditions générales d'utilisation

*Dernière mise à jour : 22 août 2026*

## 1. Objet

Les présentes conditions régissent l'utilisation du simulateur accessible
depuis ce site. L'utilisation du site vaut acceptation de ces conditions.

## 2. Accès

Le service est gratuit, ouvert à tous, et ne requiert la création d'aucun compte.
L'éditeur peut le modifier, le suspendre ou l'interrompre à tout moment, sans
préavis et sans indemnité. Il ne garantit aucune disponibilité.

## 3. Nature du service — stade de développement

**Le service est en cours de construction et son modèle de calcul est en cours
d'affinage.** Il est mis à disposition dans l'état où il se trouve, à des fins
d'information et de pédagogie. Certaines valeurs réglementaires n'ont pas encore
été vérifiées à leur source officielle et sont signalées comme telles dans
l'interface.

Le service ne constitue ni un conseil en investissement, ni un conseil fiscal, ni
une offre de crédit, ni un service d'intermédiation bancaire. L'avertissement
complet fait partie intégrante des présentes conditions.

## 4. Responsabilité

Les résultats sont produits automatiquement à partir des paramètres saisis par
l'utilisateur et d'hypothèses que celui-ci choisit. **L'utilisateur demeure seul
responsable des décisions qu'il prend.**

L'éditeur ne peut être tenu responsable d'un dommage résultant de l'utilisation des
résultats, notamment en cas d'erreur de calcul, de valeur réglementaire périmée, ou
d'écart avec le calcul d'un établissement bancaire. Il s'engage en revanche à
corriger sans délai toute erreur qui lui est signalée.

## 5. Obligations de l'utilisateur

L'utilisateur s'engage à ne pas perturber le fonctionnement du service, ni à en
extraire massivement le contenu par des moyens automatisés à des fins commerciales.

## 6. Données personnelles

Voir la politique de confidentialité. Le service ne collecte aucune donnée saisie
par l'utilisateur.

## 7. Droit applicable

Les présentes conditions sont soumises au droit français. À défaut de résolution
amiable, le litige relève des juridictions françaises compétentes.

> **Note de rédaction.** La clause de médiation de la consommation a été retirée le
> 22 août 2026 : elle n'est obligatoire que pour un professionnel, et le régime
> retenu est celui de l'éditeur non professionnel. L'y laisser aurait annoncé un
> recours qui n'existe pas. À rétablir si une activité commerciale apparaît.
